...
import ProductSearchItemAdd from "./ProductSearchItemAdd";


const AddProducts =  ({subProducts, setSubProducts, products, toggleModal}) => {
    ...
    function renderItem(product) {
        ...
        <div class="productSearchItem" data-name={`${product.title} ${product.id}`} style={{borderBottom:'1px solid #e1e3e5'}}>
            <ResourceItem id={product.id}>       
                <ProductSearchItemAdd toggleModal={toggleModal} subProducts={subProducts} setSubProducts={setSubProducts} product={product}></ProductSearchItemAdd>      
            </ResourceItem>
        </div>
        );
    }
